...
export default function AdminPage({
  ...
  const [newMoodOption, setNewMoodOption] = useState('');
    ...
    async function loadMoods() {
      try {
        if (isSupabaseConfigured()) {
          ...
          if (stored) {
            setMoodChoices(JSON.parse(stored));
          }
        }
      } catch (err) {
        console.error(err);
  ...
  const handleAddMood = async (e) => {
    e.preventDefault();
    const trimmed = newMoodOption.trim();
    if (!trimmed) return;
    if (moodChoices.includes(trimmed)) {
      setMoodSaveStatus('Option already exists.');
      return;
    }

    const updatedChoices = [...moodChoices, trimmed];
    setMoodChoices(updatedChoices);
    setNewMoodOption('');
    await saveMoodChoices(updatedChoices);
  };

  const handleDeleteMood = async (optionToDelete) => {
    const updatedChoices = moodChoices.filter(c => c !== optionToDelete);
    setMoodChoices(updatedChoices);
    await saveMoodChoices(updatedChoices);
  };

  const saveMoodChoices = async (choices) => {
    ...
    try {
      ...
      localStorage.setItem('bahattor_mood_choices', JSON.stringify(choices));
      syncToBridge('bahattor_mood_choices', JSON.stringify(choices));
      setMoodSaveStatus('Mood options saved successfully!');
      setTimeout(() => setMoodSaveStatus(''), 2000);
    } catch (err) {
      console.error(err);
      setMoodSaveStatus('Failed to save mood options: ' + err.message);
    }
  };
    ...
    <div className="admin-container">
      ...
      <section className="admin-section">
        <h3 className="admin-section-title">
          ...
          Manage Today's Mood Options
        </h3>
        
        <form onSubmit={handleAddMood} style={{ marginBottom: '20px' }}>
          <div className="admin-form-grid" style={{ gridTemplateColumns: '1fr auto', alignItems: 'end', gap: '12px' }}>
            <div className="admin-form-group">
              <label className="admin-label" htmlFor="mood-input">Add Mood Option</label>
              <input
                id="mood-input"
                className="admin-input"
                type="text"
                ...
                value={newMoodOption}
                onChange={e => setNewMoodOption(e.target.value)}
                required
              />
            </div>
            <button type="submit" className="admin-btn">Add Option</button>
          </div>
          {moodSaveStatus && (
            <div style={{ marginTop: '8px', fontSize: '13px', color: 'var(--accent)', fontWeight: '500' }}>
              {moodSaveStatus}
            </div>
          )}
        </form>
        ...
        <div style={{ fontSize: '13px', color: 'var(--text-tertiary)', marginBottom: '10px', fontFamily: 'var(--font-mono)' }}>
          Active Mood Options ({moodChoices.length})
        </div>

        <div className="admin-folders-list">
          {moodChoices.map((choice, idx) => (
            <div key={idx} className="admin-folder-row-item">
              <span style={{ fontSize: '15px', fontWeight: '500' }}>{choice}</span>
              <div className="admin-actions">
                <button
                  type="button"
                  className="admin-icon-btn delete"
                  onClick={() => handleDeleteMood(choice)}
                  aria-label="Delete mood option"
                  title="Delete"
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <polyline points="3 6 5 6 21 6" />
